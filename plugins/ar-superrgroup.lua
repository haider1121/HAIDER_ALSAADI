...
local function check_member_super(cb_extra, success, result)
  ...
  if success == 0 then
  send_large_msg(receiver, "لاتضل تكمز🐯وتلعب بكيفك")
  end
  for k,v in pairs(result) do
    ...
    moderators = {},
      ...
      save_data(_config.moderation.data, data)
    local text = 'انا الافضل 😌انا الاقوى💪وسوف اكون كد المسؤلية😎'
...
end—Get and output info about supergroup
...
local function lock_group_links(msg, data, target)
  ...
  return '🔐عزيزي😻 الغالي الروابط🚩 تم قفلها'
  else
    data[tostring(target)]['settings']['lock_link'] = 'yes'
    save_data(_config.moderation.data, data)
    return '🚺تاج👑 راسي تم قفل🔐 الروابط '
  end
end
...
local function unlock_group_links(msg, data, target)
  ...
  if group_link_lock == 'no' then
    return '🔓عزيزي😻 الغالي الروابط🚩 مفتوحة'
  else
    data[tostring(target)]['settings']['lock_link'] = 'no'
    save_data(_config.moderation.data, data)
    return '🚺تاج👑 راسي الروابط🔓 مفتوحة'
  end
end
...
local function lock_group_spam(msg, data, target)
  ...
  if not is_owner(msg) then
    return "😎لاتلعب😕 بكيفك ولاتصير دودة🐛 بس المدير والمشرف😌 يكدر"
  end
  ...
  if group_spam_lock == 'yes' then
    ...
    save_data(_config.moderation.data, data)
    return 'عزيزي😻الغالي تم🚩 قفل🔐 الكلايش '
  end
end
...
local function unlock_group_spam(msg, data, target)
  ...
  if group_spam_lock == 'no' then
    return '🚺تاج👑 راسي الكلايش🔓مفتوحة 'else
...
local function lock_group_flood(msg, data, target)
  ...
  if group_flood_lock == 'yes' then
    return 'عزيزي😻الغالي تم🚩 قفل🔐التكرار'
  else
    data[tostring(target)]['settings']['flood'] = 'yes'
    save_data(_config.moderation.data, data)
    return 'عزيزي😻الغالي تم🚩فتح التكرار🔓'
  end
end
...
local function lock_group_sticker(msg, data, target)
  ...
  if group_sticker_lock == 'yes' then
    return 'عزيزي😻الغالي تم🚩قفل الملصقات🔐'
  else
    data[tostring(target)]['settings']['lock_sticker'] = 'yes'
    save_data(_config.moderation.data, data)
    return 🚺'تاج👑راسي تم قفل🔐 الملصقات'
  end
endlocal function unlock_group_sticker(msg, data, target)
  if not is_momod(msg) then
    return
  end
  local group_sticker_lock = data[tostring(target)]['settings']['lock_sticker']
  if group_sticker_lock == 'no' then
    return 'عزيزي😻الغالي تم🚩مفتوحة الملصقات🔓'
  else
    data[tostring(target)]['settings']['lock_sticker'] = 'no'
    save_data(_config.moderation.data, data)
    return '🚺تاج😍 راسي الملصقات🔓 مفتوحة'
  end
end
...
local function set_public_membermod(msg, data, target)
  if not is_momod(msg) then
    return "🚶 💨 لاتلعب بكيفك ابو حلك روح ازحف احسن الك👋👮"
  end
...
endlocal function unset_public_membermod(msg, data, target)
  if not is_momod(msg) then
    return
  end
  local group_public_lock = data[tostring(target)]['settings']['public']
...
local function promote2(receiver, member_username, user_id)
  ...
  data[group]['moderators'][tostring(user_id)] = member_tag_username
  save_data(_config.moderation.data, data)
  send_large_msg(receiver, member_username..💭'تم رفعك🏋🏻 ادمن لاتحس روحك قيس👴🏼 الخزعلي ترة👁‍🗨 تنطرد.')
endlocal function demote2(receiver, member_username, user_id)
  local data = load_data(_config.moderation.data)
  local group = string.gsub(receiver, 'channel#id', '')
  if not data[group] then
    return send_large_msg(receiver, '✋🏻❌ الَمَجموَعهْ ليستَ فعأَلهَ ❗️.')
  ...
  data[group]['moderators'][tostring(user_id)] = nil
  save_data(_config.moderation.data, data)
  send_large_msg(receiver, member_username..' 💭هاا🐛 شكلتلك لاتحس روحك قيس الخز👴🏼 علي تم تنزيلك👁‍🗨 من الادمنية.')
end
...
function get_message_callback(extra, success, result)
  ...
  elseif get_cmd == 'channel_block' and result.action and result.action.type == 'chat_add_user' then
    ...
    if is_admin2(member_id) then
         return send_large_msg("channel#id"..channel_id, "❌ لا تمسلت  بكيفك لايمكنك طرد الاداري❗️")
    endsavelog(msg.to.id, name_log.." ["..msg.from.id.."] kicked: ["..user_id.."] by reply to sev. msg.")
  ...
  elseif get_cmd == "رفع اداري" then
    ...
    if result.from.username then
      text = "@"..result.from.username.." بَعـدَ شـتريَدْ 😍 منَ ربـَكَ تَمَ رفعَكَ فَي ألآَدرهَ 💪"
    else
      text = "[ "..user_id.." ]بَعـدَ شـتريَدْ 😍 منَ ربـَكَ تَمَ رفعَكَ فَي ألآَدرهَ 💪"
    end
  ...
  elseif get_cmd == "تنزيل اداري" then
    ...
    if result.from.username then
      text = "@"..result.from.username.." خطـيهَ تمَ تنزيله 😢 منْ ألآَدأرهَ لتبجي جرَأَرهَ كلـبيَ 💔"
    else
      text = "[ "..user_id.." ] خطـيهَ تمَ تنزيله 😢 منْ ألآَدأرهَ لتبجي جرَأَرهَ كلـبيَ 💔"
    end
  ...
  elseif get_cmd == 'mute_user' then
    if result.service thenlocal action = result.action.type
      if action == 'chat_add_user' or action == 'chat_del_user' or action == 'chat_rename' or action == 'chat_change_photo' then
        if result.action.user then
          user_id = result.action.user.peer_id
        end
    ...
    elseif is_admin1(msg) then
      ...
      send_large_msg(receiver, " ["..user_id.."] تم كتمك🚷 طبك/طبج مرض 😏عساج ملصومة للصبح🌕")
    end
  end
end
...
end— Begin resolve username actions
local function callbackres(extra, success, result)
  ...
  elseif get_cmd == 'mute_user' then
    local user_id = result.peer_id
    local receiver = extra.receiverlocal chat_id = string.gsub(receiver, 'channel#id', '')
    if is_muted_user(chat_id, user_id) then
      unmute_user(chat_id, user_id)
...
elseif get_cmd == "رفع اداري" then
   for k,v in pairs(result) do
    ...
    local user_id = v.peer_id
    promote_admin(receiver, member_username, user_id)end
    send_large_msg(channel_id, text)
    return
 end
 elseif get_cmd == 'رفع المدير' then
...
local function run(msg, matches)
  ...
  local data = load_data(_config.moderation.data)
    ...
    if matches[1] == "الاداريين" then
      ...
      member_type = '❤️ قأئـمَهَ أَلأدرييــنَ ❤️'
    ...
    endif matches[1] == 'مسح' and is_momod(msg) then
    ...
    endif matches[1] == 'تغير الرابط' and is_momod(msg)then
    ...
    end]]if matches[1] == 'رفع اداري' then
    ...
    endif matches[1] == 'رفع المدير' and is_owner(msg) then
      if type(msg.reply_id) ~= "nil" then
        local cbreply_extra = {
          get_cmd = 'رفع المدير',
          msg = msg
    ...
    if matches[1] == 'md' and is_sudo(msg) then
      ...
      return "ok"
    endif matches[1] == 'تنزيل ادمن' then
      if not is_momod(msg) then
        return
      end
      if not is_owner(msg) then
    ...
    endif matches[1] == 'قفل' and is_momod(msg) then
      local target = msg.to.id
    ...
    if matches[1] == 'فتح' and is_momod(msg) then
      local chat_id = msg.to.id
      if matches[2] == 'الصوت' then
      local msg_type = 'Audio'if is_muted(chat_id, msg_type..': yes') then
    ...
    endif matches[1] == "كتم" and is_momod(msg) then
      local chat_id = msg.to.id
      local hash = "mute_user"..chat_id
      local user_id = ""
      if type(msg.reply_id) ~= "nil" then
    ...
    if matches[1] == 'msg.to.id' and is_admin1(msg) then
      ...
      reply_msg(msg.id, text, ok_cb, false)
      post_large_msg(receiver, text)
    end—Admin Join Service Message
    if msg.service then
